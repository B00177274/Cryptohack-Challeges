#!/usr/bin/env python3

from Crypto.Util.number import getPrime, bytes_to_long, inverse
from random import getrandbits
from math import gcd

FLAG = b"crypto{?????????????????????????????????????}"

m = bytes_to_long(FLAG)

def get_huge_RSA():
    p = getPrime(1024)
    q = getPrime(1024)
    N = p*q
    phi = (p-1)*(q-1)
    while True:
        d = getrandbits(512)
        if (3*d)**4 > N and gcd(d,phi) == 1:
            e = inverse(d, phi)
            break
    return N,e


N, e = get_huge_RSA()
c = pow(m, e, N)

print(f'N = {hex(N)}')
print(f'e = {hex(e)}')
print(f'c = {hex(c)}')

N = 
e = 
c = 

import owiener
from Crypto.Util.number import long_to_bytes
N = int('',16)
e = int('',16)
c = int('',16)
d = owiener.attack(e,N)
m = pow(c,d,N)
print(long_to_bytes(m))
#Solution
#The script generates large RSA keys by selecting two 1024-bit prime numbers and computing the modulus N and the totient phi.
#It then calculates the public exponent e and the corresponding private exponent d by ensuring that the greatest common divisor of d and phi is 1, and that d is sufficiently large
#The ciphertext c is generated by encrypting the message using the public exponent e and modulus N
#Using Wiener’s attack, the script recovers the private exponent d, decrypts the ciphertext c, and prints the original plaintext message.
